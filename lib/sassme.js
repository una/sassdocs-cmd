...
app.get('/scrape', function(req, res){
    // ended off here: https://scotch.io/tutorials/scraping-the-web-with-node-js#traversing-the-dom
...
program
  ...
  .option('@each', '@each directive')
  .option('@for', '@for directive')
  .option('@while', '@while directive')
  .option('@mixin', 'Defining and using mixins')
  .option('function-directives', 'Define your own Sass functions')
  .option('output-style', 'Output styles overview')
  .option(':nested', 'Nested output')
  .option(':expanded', 'Expanded output')
  .option(':compact', 'Compact output')
  .option(':compressed', 'Compressed output')
  .option('extending-sass', 'Taking it up a notch')
  .parse(process.argv);
...
// get the arguments, minus node
	...
	else if (['@if', '@-if'].indexOf(args) > -1) {
		console.log(out_atif);
	}

	else if (['@for', '@-for'].indexOf(args) > -1) {
		console.log(out_atfor);
	}

	else if (['@each', '@-each'].indexOf(args) > -1) {
		console.log(out_ateach);
	}

	else if (['@while', '@-while'].indexOf(args) > -1) {
		console.log(out_atwhile);
	}

	else if (['@mixin', '@-mixin'].indexOf(args) > -1) {
		console.log(out_atmixin);
	}

	else if (['function-directives', 'function-directive'].indexOf(args) > -1) {
		console.log(out_funcdirec);
	}

	else if (['output-styles', 'output-style', 'output', 'outputs'].indexOf(args) > -1) {
		console.log(out_output);
	}

	else if ([':nested'].indexOf(args) > -1) {
		console.log(out_out_nested);
	}

	else if ([':expanded'].indexOf(args) > -1) {
		console.log(out_out_expanded);
	}

	else if ([':compact'].indexOf(args) > -1) {
		console.log(out_out_compact);
	}

	else if ([':compressed'].indexOf(args) > -1) {
		console.log(out_out_compressed);
	}

	else if (['extending-sass'].indexOf(args) > -1) {
		console.log(out_extending);
	}
